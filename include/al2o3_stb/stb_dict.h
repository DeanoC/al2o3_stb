...
#define stb_declare_hash(PREFIX, TYPE, N, KEY, VALUE) \
   ...
   PREFIX int STB__(N, init)(TYPE *h, size_t count);\
...
{                                                                             \
   ...
   h->table = (STB_(N,_hashpair)*) STBI_MALLOC(sizeof(h->table[0]) * count);  \
...
PREFIX TYPE * STB__(N, create)(void)                                          \
{                                                                             \
   TYPE *h = (TYPE *) STBI_MALLOC(sizeof(*h));                                \
   if (h) {                                                                   \
      if (STB__(N, init)(h, 16))                                              \
         h->alloced = 1;                                                      \
      else { STBI_FREE(h); h=NULL; }                                          \
   }                                                                          \
   return h;                                                                  \
}                                                                             \
                                                                              \
...
{                                                                             \
   ...
   TYPE *h = (TYPE *) STBI_MALLOC(sizeof(*h));                                \
   if (!h) return NULL;                                                       \
   if (!STB__(N, init)(h, a->limit)) { STBI_FREE(h); return NULL; }           \
   h->count = a->count;                                                       \
   h->deleted = a->deleted;                                                   \
   h->alloced = 1;                                                            \
   h->ev = a->ev; h->dv = a->dv;                                              \
...
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictPtr, stb_dictPtr_, void *, void *)
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictI32, stb_dictI32_, int32_t, int32_t)
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictU32, stb_dictU32_, uint32_t, uint32_t)
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictI64, stb_dictI64_, int64_t, int64_t)
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictU64, stb_dictU64_, uint64_t, uint64_t)
stb_declare_fixed_sized_hash(AL2O3_EXTERN_C, stb_dictSizeT, stb_dictSizeT_, size_t, size_t)
